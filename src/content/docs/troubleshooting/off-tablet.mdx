...
<Aside type="caution" title="Samsung Browser">Do **not** use the default Samsung browser. Tablets are configured to keep their screen on only if Chrome is used, so if Samsung browser is in use the tablet will turn off again!</Aside>
